...
describe("/services", () => {
    ...
    test("POST /users/services - Must be able to create a service", async () => {
        ...
        const response = await request(app).post("/users/services").send(mockedService1).set("Authorization", `Bearer ${loginResponse.body.token}`);
    ...
    test("GET users/:userId/services - It should be possible to list all services for a single user", async () => {
        ...
        const response = await request(app).get(`/users/${users.body[0].id}/services`).set("Authorization", `Bearer ${loginAdmResponse.body.token}`);
    ...
    test("PATCH users/:userId/services/:servicesId - Should not be able to update a service with invalid id", async () => {
        ...
        const response = await request(app).patch(`/users/${findUser.body[0].id}/services/13970660-5dbe-423a-9a9d-5c23b37943cf`).send(mockedUpdateService1).set("Authorization", `Bearer ${loginUserResponse.body.token}`);
    ...
    test("DELETE users/:userId/services/:servicesId - It must be possible to delete a service", async () => {
        ...
        const response = await request(app).delete(`/users/${findUser.body[0].id}/services/${findService.body[0].id}`).set("Authorization", `Bearer ${loginUserResponse.body.token}`);
    ...
    test("DELETE users/:userId/services/:servicesId - Should not be able to delete a service with invalid id", async () => {
        ...
        const response = await request(app).delete(`/users/${findUser.body[0].id}/services/13970660-5dbe-423a-9a9d-5c23b37943cf`).set("Authorization", `Bearer ${loginUserResponse.body.token}`);
