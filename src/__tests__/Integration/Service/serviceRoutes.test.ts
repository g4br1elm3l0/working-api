...
describe("/services", () => {
    ...
    test("GET /users/services - It should be able to list all services", async () => {
        ...
        const loginWorkerRespone = await request(app).post("/login").send(mockedUserWorkerLogin);
        const response = await request(app).get("/users/services").set("Authorization", `Bearer ${loginWorkerRespone.body.token}`);
    ...
    test("GET users/services/:servicesId - Must be able to list a service", async () => {
        ...
        const service = await request(app).get("/users/services").set("Authorization", `Bearer ${loginRespone.body.token}`);
        const response = await request(app).get(`/users/services/${service.body[0].id}`).set("Authorization", `Bearer ${loginRespone.body.token}`);
    ...
    test("GET users/:userId/services - You shouldn't be able to list a service if you're not an administrator or owner", async () => {
        const loginRespone = await request(app).post("/login").send(mockedUserWorkerLogin);        
        const users = await request(app).get("/users").set("Authorization", `Bearer ${loginRespone.body.token}`);
        const response = await request(app).get(`/users/${users.body[0].id}/services`).set("Authorization", `Bearer ${loginRespone.body.token}`);
